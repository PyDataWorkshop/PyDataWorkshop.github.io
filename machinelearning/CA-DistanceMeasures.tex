...
%--------------------------------------------------------------------------------------%
<p>
#### {\texttt{R} - Distance Measures supported by the \texttt{dist} Function}

 The \texttt{dist} function computes and returns the distance matrix computed by using the specified distance measure to compute the distances between the rows of a data matrix.
 
The distance measures supported by \texttt{dist} are


 \texttt{euclidean} (but not squared euclidean directly)
 \texttt{maximum}
 \texttt{manhattan}
 \texttt{canberra}
 \texttt{binary}* \texttt{minkowski}.
<p>
<p>
...
\end{table}
 ...
 The next closest pair (4.82) are 1 and 2. So this is the next linkage. [ So far (3,8,9) and (2,10) ]* The next closest pair (4.84) are 2 and 10. The next linkage joins case 1 to 2 and 10.
